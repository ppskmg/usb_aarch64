...
> flash USB
> SSD USB (NVME m2)
> Устройство для доступа по SSH (В моём случае iPad PRO + blink)
> Устройство для записи образа (В моём случае Android)
> Клавиатура + мыш
> ethernet
> Монитор 

### Запись образа через телефон:
Вставить кардридер или флешку в телефон
Установить приложение Raspi Card Imager:
Setting > Format SD
Setting > Choice OS > Raspberry Pi OS light
Выбрать SD

Write to SD

### Установка:
Извлечь SD из телефона
Вставить SD в Raspberry PI
Подключить переферию (мыш, клавиатура, ethernet, монитор)
Включить питание RPI
(Если при включении на экране RGB цвета и не чего не происходит, то отключить монитор и подключить снова)
Пройти процедуру установки
...
Вставить usb флешку
воспользоваться инструкцией по установке ARMV7, но с файлом для AArch64
...
Чтобы при при перезагрузке без SD, загрузится с flash USB
```
..mmcblk0.. boot
```
на
...
в случае, если система установлена на SD карту
```
